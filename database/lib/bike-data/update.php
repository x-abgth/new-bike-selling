		...
		if ($a_file_error === 0 && $b_file_error === 0 && $c_file_error === 0) {
			if($a_fileSizeInMB > 1 && $b_fileSizeInMB > 1 && $c_fileSizeInMB > 1) {
				...
				if(in_array($a_file_ext_lc, $allowed_ext) && in_array($b_file_ext_lc, $allowed_ext) && in_array($c_file_ext_lc, $allowed_ext)) {
					...
					$sql = "UPDATE `bikes` SET `m_name`='$m_name',`m_color`='$m_color',`m_category`='$m_category',`m_mileage`='$m_mileage',`m_displacement`='$m_displacement',`m_engine`='$m_engine',`m_cylinders`='$m_cylinders',`m_power`='$m_power',`m_torque`='$m_torque',`m_fuel_tank`='$m_fuel_tank',`m_abs`='$m_abs',`m_key_features`='$m_key_features',`m_image_a`='$a_filename', `m_image_b`='$b_filename', `m_image_c`='$c_filename', `m_price`='$m_price_value' WHERE id='$id_edit'";
					...
					if(mysqli_query($link, $sql)) {
						...
						$sm = 'Record updated successfully!';
						echo ("<script LANGUAGE='JavaScript'>
			    			window.alert('$sm');
			   				window.location.href='admin_view.php';
			    			</script>");
